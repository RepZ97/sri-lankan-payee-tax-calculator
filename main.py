...
class GovernmentsCutCalc(FloatLayout):
    def __init__(self, **kwargs):
        ...
        parent_layout = BoxLayout(orientation='vertical', size_hint=(None, None), size=(120, 210),
        ...
        hist_button_layout = BoxLayout(size_hint=(None, None), size=(120, 30), pos_hint={'center_x': 0.17}, padding=(0, 60, 0, 0))
        ...
        # ** EPF Deduction Slider Section **
        etf_layout = BoxLayout(orientation='vertical', size_hint=(None, None), size=(300, 80),
                               pos_hint={'center_x': 0.5})

        self.etf_label = Label(text="Salary Percentage Eligible \nfor EPF Deductions: 60%", size_hint=(None, None), size=(200, 50))
        etf_layout.add_widget(self.etf_label)

        self.etf_slider = Slider(min=0, max=1, value=0.6, step=0.1, size_hint=(None, None), size=(205, 30), pos_hint={'center_x': 0.33})
    ...
    def update_etf_label(self, instance, value):
        """ Update the EPF percentage label when slider moves """
        self.etf_label.text = f"Salary Percentage Eligible \nfor EPF Deductions: {int(value * 100)}%"

    def calculate_take_home(self, instance):
        try:
            gross_salary = float(self.salary_input.text)
            selected_version = self.version_spinner.text
            etf_percentage = self.etf_slider.value  # Get user-selected EPF deduction percentage
            take_home, total_tax, etf_deduction = self.compute_salary(gross_salary, selected_version, etf_percentage)
